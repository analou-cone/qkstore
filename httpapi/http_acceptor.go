...
import (
	...
	"github.com/analou-cone/qkstore"
)

// AcceptorServer wraps a qkstore.Acceptor and provides a basic HTTP API.
...
// AcceptorClient implements qkstore.Acceptor by making HTTP requests to a
...
// Prepare implements qkstore.Acceptor by making an HTTP request to the remote
// acceptor API.
func (ac AcceptorClient) Prepare(ctx context.Context, key string, b qkstore.Ballot) (value []byte, current qkstore.Ballot, err error) {
	...
	if err != nil {
		return nil, qkstore.Ballot{}, errors.Wrap(err, "constructing HTTP request")
	}
	...
	if err != nil {
		return nil, qkstore.Ballot{}, errors.Wrap(err, "executing HTTP request")
	}

	current, err = header2ballot(resp.Header)
	if err != nil {
		return nil, qkstore.Ballot{}, errors.Wrap(err, "extracting response ballot")
	}
...
// Accept implements qkstore.Acceptor by making an HTTP request to the remote
// acceptor API.
...
func header2ballot(h http.Header) (qkstore.Ballot, error) {
	ballot := h.Get(ballotHeaderKey)
	if ballot == "" {
		return qkstore.Ballot{}, fmt.Errorf("%s not provided", ballotHeaderKey)
	}
	tokens := strings.SplitN(ballot, "/", 2)
	if len(tokens) != 2 {
		return qkstore.Ballot{}, fmt.Errorf("%s has invalid format", ballotHeaderKey)
	}
	counter, err := strconv.ParseUint(tokens[0], 10, 64)
	if err != nil {
		return qkstore.Ballot{}, fmt.Errorf("%s has invalid Counter value %q", ballotHeaderKey, tokens[0])
	}
	id, err := strconv.ParseUint(tokens[1], 10, 64)
	if err != nil {
		return qkstore.Ballot{}, fmt.Errorf("%s has invalid ID value %q", ballotHeaderKey, tokens[1])
	}
	return qkstore.Ballot{Counter: counter, ID: id}, nil
}

func ballot2header(b qkstore.Ballot, h http.Header) {
